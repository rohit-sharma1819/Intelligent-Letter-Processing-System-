...
while ($row = $result->fetch_assoc()) {
    ...
    $seenClass = ($row['seen'] == 0) ? 'not-seen' : 'seen';  // Add classes based on seen status
    ...
    echo "<tr class='$seenClass'>
                ...
                <td>{$row['received_by']}</td>
                <td>{$row['action_taken']}</td>
                <td><a href='{$row['letter_upload_path']}' target='_blank'>
                    <img src='{$row['letter_upload_path']}' alt='Letter Upload' style='width: 100px; height: auto;'>
                </a></td>
                <td>{$row['created_at']}</td>
                <td>";

    // Check the 'permission' value and display accordingly
    // if ($row['permission'] == 1) {
    //     echo "Permitted"; // Display "Permitted" if the permission is 1
    // } else {
    //     echo "Not Permitted"; // Display "Not Permitted" if the permission is 0
    // }
